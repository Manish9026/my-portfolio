...
function SampleNextArrow(props) {
  ...
  return (
    <div
      className="w-14 next h-12 bg-[#0c1821] hover:bg-black duration-300 rounded-md text-2xl text-gray-400 flex justify-center items-center absolute -top-16 right-0 shadow-shadowOne cursor-pointer z-10"
      onClick={onClick}
    >
      <HiArrowRight />
    </div>
...
function SamplePrevArrow(props) {
  ...
  return (
    <div
      className="w-14 h-12 prev bg-[#0c1821] hover:bg-black duration-300 rounded-md text-2xl text-gray-400 flex justify-center items-center absolute -top-16 left-0 shadow-shadowOne cursor-pointer z-10"
      onClick={onClick}
    >
      <HiArrowLeft />
    </div>
...
const Testimonial = () => {
  ...
  return (
    <section
      id="testimonial"
      className="w-full  border-b-[1px] border-b-line primary-p"
    >
...
<SamplePrevArrow />
        <Swiper
          className="border rounded-md border-slate-600 w-full max-h-[70%] py-10 bg-gradient-to-r from-[#1d154b] to-[#1d154b] p-4 lgl:p-8 flex flex-col gap-4 lgl:gap-8"
        ...
        >
          {testimonialData.map(
            ({ des, post, workThrough, workTitle, name, nationality, rating, src }, id) => (
              <SwiperSlide key={id}>
                <div className=" rounded-md  w-full max-h-[70%] py-10 bg-gradient-to-r from-[#1d154b] to-[#1d154b] p-4 lgl:p-8 flex flex-col gap-4 lgl:gap-8">
                  <div className="flex flex-wrap justify-start sml:justify-start lgl:items-center pb-6 gap-4 border-b-2 border-b-gray-700">
                    <img
                      src={src}
                      alt=""
                      className="aspect-video rounded-md object-contain max-w-[250px]"
                    />
                    <div>
                      <h2 className="capitalize text-xl lgl:text-2xl font-medium tracking-wide">{name}</h2>
                      <p className="text-base text-gray-400">{post}</p>
                      ...
                      <p className="text-base text-gray-400 ">{workThrough}</p>
                      <div className="text-yellow-500 flex gap-1">
                        {Array(rating)
                          .fill(null)
                          .map((_, idx) => (
