...
const Navbar = ({navType=""}) => {
  ...
  return (
    <div className="w-full primary-p nav h-24 sticky top-0 z-50 bg-bodyColor mx-auto flex justify-between items-center font-titleFont border-b-[1px] border-b-gray-600">
              ...
              <ul className="flex flex-col gap-4 ">
                {navLinksdata.map((item) => (
                  <li
                    ...
                    className="text-base font-normal text-gray-400 tracking-wide cursor-pointer hover:text-designColor duration-300"
                  >
                    {navType=="link"?(<Link to={`/#${item.link}`} onClick={() => setShowMenu(false)  }>
                     {item.title}
